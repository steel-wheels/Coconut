...
public class CNAppleEventManager
{
	...
	public func property(forKey key: String) -> Any? {
		...
		case "foregroundColor":
			...
			return vpref.terminalForegroundColor()
		case "backgroundColor":
			let vpref = CNPreference.shared.viewPreference
			return vpref.terminalBackgroundColor
		case "terminalHeight":
	...
	open func setProperty(_ value: Any?, forKey key: String) {
		...
		case "foregroundColor":
			if let col = value as? CNColor {
				let vpref = CNPreference.shared.viewPreference
				vpref.setTerminalForegroundColor(color: col)
				hasset = true
			}
		case "backgroundColor":
			if let col = value as? CNColor {
				let vpref = CNPreference.shared.viewPreference
				vpref.setTerminalBackgroundColor(color: col)
				hasset = true
			}
		case "terminalHeight":
