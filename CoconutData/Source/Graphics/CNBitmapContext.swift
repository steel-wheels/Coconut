...
{
	public static let InterfaceName = "BitmapContextIF"
	...
	static func allocateInterfaceType(colorIF colif: CNInterfaceType) -> CNInterfaceType {
		...
		let col: CNValueType = .interfaceType(colif)
		let members: Array<M> = [
			M(name: "width", 	type: .numberType),
			M(name: "wheight",	type: .numberType),
			M(name: "get",		type: .functionType(col, [.numberType, .numberType])),
			M(name: "set", 		type: .functionType(.voidType, [.numberType, .numberType, col]))
		]
