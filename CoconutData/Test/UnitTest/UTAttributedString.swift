...
public func testAttributedString(console cons: CNConsole) -> Bool
{
	...
	let result2 = testShell(console: cons)
...
private func testShell(console cons: CNConsole) -> Bool
{
	let font     = CNFont.systemFont(ofSize: 10.0)
	let terminfo = CNTerminalInfo(width: 80, height: 20)
	let astr     = NSMutableAttributedString(string: "", font: font, terminalInfo: terminfo)

	var index = 0

	cons.print(string: "* testShell\n")
	cons.print(string: "init   : index=\(index), \"\(astr.string)\"\n")

	let spaces = String(repeating: " ", count: 3)
	index = astr.insert(string: spaces, at: index, font: font, terminalInfo: terminfo)
	cons.print(string: "insert : index=\(index), \"\(astr.string)\"\n")

	index = astr.write(string: "abc", at: index, font: font, terminalInfo: terminfo)
	cons.print(string: "write  : index=\(index), \"\(astr.string)\"\n")

	index = astr.insert(string: "-", at: index, font: font, terminalInfo: terminfo)
	cons.print(string: "insert : index=\(index), \"\(astr.string)\"\n")

	index = astr.write(string: "A", at: index, font: font, terminalInfo: terminfo)
	cons.print(string: "write  : index=\(index), \"\(astr.string)\"\n")
